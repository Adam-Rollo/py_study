...
def train():
    ...
    # 实例化一个词典，之后传入x_text，让其中每个词都对应了一个ID，比如[[i love you],[i hate you]]就变成了[[1,2,3],[1,4,3]]
    ...
    # Randomly shuffle data, 种子是10，按照y的长度生成一个统一的随机索引，然后以索引用相同的顺序打乱x和y
    np.random.seed(10)
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            rnn = RNN(
                # array([[4719,   59,  182, ...,    0,    0,    0], [ 129, 7044,  284, ...,    0,    0,    0], [ 146,    3,  453, ...,    0,    0,    0], ...,
                sequence_length=x_train.shape[1],  # 100列
                num_classes=y_train.shape[1], #二分类
            ...
            # 每当程序处理一个batch的时候，global_step将会自动加一，这个数值除了做计数器，有时候会放到学习速率的函数中，让学习速度随着batch的个数减少
